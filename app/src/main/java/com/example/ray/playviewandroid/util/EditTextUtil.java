...
public class EditTextUtil {
	...
	public static void showKeyboard(Context context, EditText et, View toGetWindowTokenView, boolean show){
		if (context == null) {
			LogUtil.d(TAG, "showKeyboard  context == null >> return;");
			return;
		}
		...
		if (toGetWindowTokenView == null) {
			LogUtil.w(TAG, "showKeyboard   toGetWindowTokenView == null");
			toGetWindowTokenView = et;
		}
		if (toGetWindowTokenView == null) {
			LogUtil.d(TAG, "showKeyboard  toGetWindowTokenView == null && et == null  >> return;");
			return;
		}
	...
	public static boolean isInputedCorrect(Activity context, EditText et, int type, int stringResId) {
		try {
			...
			LogUtil.d(TAG, "isInputedCorrect try { if (context != null && stringResId > 0) {catch (Exception e) \n" + e.getMessage());
		}
		return false;
	}
	/**判断edittext输入文字是否合法
	...
	public static boolean isInputedCorrect(Activity context, EditText et, int type, String errorRemind) {
		if (context == null || et == null) {
			LogUtil.d(TAG, "isInputedCorrect context == null || et == null >> return false;");
			return false;
		}
		...
		switch (type) {
			...
			case TYPE_PASSWORD:
				...
				if (!StringUtil.isNumberOrAlpha(inputed)) {
			...
			case TYPE_PHONE:
				...
				if (!StringUtil.isPhone(inputed)) {
					CommonUtil.showShortToast(context, "您输入的手机号格式不对哦~");
					return false;
				}
				break;
			case TYPE_MAIL:
				if (!StringUtil.isEmail(inputed)) {
					return showInputedError(context, "您输入的邮箱格式不对哦~");
				}
				break;
			default:
				if (!StringUtil.isNotEmpty(inputed, true) || inputed.equals(StringUtil.getTrimedString(et.getHint()))) {
	...
	public static boolean showInputedError(Activity context, EditText et, String string) {
		if (context == null || !StringUtil.isNotEmpty(string, false)) {
			LogUtil.d(TAG, "showInputedError  context == null || et == null || StringUtil.isNotEmpty(string, false) == false >> return false;");
			return false;
		}
		if (et == null) {
			CommonUtil.showShortToast(context, StringUtil.getTrimedString(string));
		} else {
			et.setText("");
			et.setHint(string);
			et.setHintTextColor(context.getResources().getColor(R.color.colorAccent));
		}
		return false;
	}
