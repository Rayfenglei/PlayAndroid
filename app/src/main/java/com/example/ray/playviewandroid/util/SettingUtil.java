...
public final class SettingUtil {
    ...
    public static boolean getBoolean(String key, boolean defaultValue) {
        if (!isContainKey(key)) {
            LogUtil.d(TAG, "writeBoolean  isContainKey(key) == false >> return defaultValue;");
            return defaultValue;
        }
    ...
    public static void putAllBoolean(boolean[] values) {
        if (values == null || values.length != KEYS.length) {
            LogUtil.d(TAG, "putAllBoolean  values == null || values.length != KEYS.length >> return;");
            return;
        }
    ...
    public static void putBoolean(String key, boolean value) {
        ...
        if (keyIndex <= 0) {
            LogUtil.d(TAG, "writeBoolean  keyIndex <= 0 >> return;");
            return;
        }
     ...
     * 测试服务器地址
     */
    public static final String URL_SERVER_ADDRESS_TEST = "https://www.bilibili.com";//测试服务器
